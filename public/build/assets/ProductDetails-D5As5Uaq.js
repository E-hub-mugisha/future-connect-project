import{r as S,u as z,j as e,H as A,L as F}from"./app-CJlpfYPO.js";import{G as D}from"./GuestLayout-B3m7kG81.js";function P({product:r}){var m,b,h,g,v,p,x,f,u,y,j;const[o,l]=S.useState("description"),[d,c]=S.useState(0),t=z({rating:"",comment:""}),i=z({quantity:1});function R(a){a.preventDefault(),t.post(route("product.reviews.store",r.id),{preserveScroll:!0,onSuccess:()=>t.reset()})}function C(a){a.preventDefault(),i.post(route("cart.add",r.id),{preserveScroll:!0})}function B(){i.setData("quantity",Math.max(1,i.data.quantity-1))}function q(){const a=r.stock??1e3;i.setData("quantity",Math.min(a,Number(i.data.quantity)+1))}const n=r.reviews&&r.reviews.length?r.reviews.reduce((a,s)=>a+s.rating,0)/r.reviews.length:0,M=(Number(r.price)*Number(i.data.quantity||1)).toFixed(2);return e.jsxs(e.Fragment,{children:[e.jsx(A,{title:r.name}),e.jsx("style",{children:`
        :root {
          --bg-deep:    #0e1618;
          --bg-card:    #121d1f;
          --bg-raised:  #172224;
