import{r as l,_ as L,u as T,j as e,H as F,L as g}from"./app-BV2sDVKx.js";import{G as $}from"./GuestLayout-zw3aPGz3.js";const P={"user.jobs.index":"/jobs","user.jobs.show":"/jobs/:id","user.jobs.apply":"/jobs",pricing:"/pricing"},m=50,I=5*1024*1024,U=["application/pdf","application/msword","application/vnd.openxmlformats-officedocument.wordprocessingml.document"],O=/\.(pdf|doc|docx)$/i;function H({job:s,categories:N=[],recent:w=[],filters:b={},routes:k={},assetBase:S="",showSubscribeModal:z=!1}){var v,y;const d=a=>k[a]||P[a]||"#",o=a=>`${S}${a}`,E=s.apply_url||`${d("user.jobs.apply")}/${s.id}`,D=a=>a.show_url||`${d("user.jobs.show")}/${a.id}`,p=l.useRef(null),n=l.useRef(null),x=l.useRef(null),c=l.useRef(null);l.useEffect(()=>{let a=!1;return L(async()=>{const{Modal:r}=await import("./bootstrap.esm-ifhUiil8.js");return{Modal:r}},[]).then(({Modal:r})=>{a||(p.current&&(n.current=new r(p.current)),x.current&&(c.current=new r(x.current),z&&c.current.show()))}),()=>{var r,_;a=!0,(r=n.current)==null||r.dispose(),(_=c.current)==null||_.dispose()}},[]);const M=()=>{var a;return(a=n.current)==null?void 0:a.show()},j=()=>{var a;return(a=n.current)==null?void 0:a.hide()},u=()=>{var a;return(a=c.current)==null?void 0:a.hide()},t=T({cover_letter:"",resume:null,name:"",email:""}),[i,f]=l.useState({cover_letter:"",resume:"",name:"",email:""}),h=t.data.cover_letter.trim().length,R=h>=m?"#48d597":"#f07070",C=()=>{const a={cover_letter:"",resume:"",name:"",email:""};h<m&&(a.cover_letter=`Please write at least ${m} characters.`);const r=t.data.resume;return r?!O.test(r.name)&&!U.includes(r.type)?a.resume="Only PDF, DOC, or DOCX files are accepted.":r.size>I&&(a.resume="File size must not exceed 5 MB."):a.resume="Please upload your resume.",f(a),!a.cover_letter&&!a.resume},A=a=>{if(a.preventDefault(),!C()){const r=document.querySelector("#jobApplyModalPage .is-invalid");r&&r.scrollIntoView({behavior:"smooth",block:"center"});return}t.post(E,{forceFormData:!0,onSuccess:()=>{t.reset(),f({cover_letter:"",resume:"",name:"",email:""}),j()}})};return s?e.jsxs(e.Fragment,{children:[e.jsx(F,{title:s.title}),e.jsx("style",{children:`
        /* ── Tokens ─────────────────────────────────────── */
        :root {
            --bg: #0e1618;
            --surface: #131e21;
