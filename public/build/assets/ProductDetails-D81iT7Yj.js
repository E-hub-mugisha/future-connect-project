import{r as S,u as z,j as e,H as F,L as R}from"./app-C-Atdk99.js";import{G as _}from"./GuestLayout-be1venag.js";function T({product:r}){var m,b,g,h,x,v,f,p,u,y,j;const[o,l]=S.useState("description"),[d,c]=S.useState(0),t=z({rating:"",comment:""}),s=z({quantity:1});function C(a){a.preventDefault(),t.post(route("product.reviews.store",r.id),{preserveScroll:!0,onSuccess:()=>t.reset()})}function q(a){a.preventDefault(),s.post(route("cart.add",r.id),{preserveScroll:!0})}function B(){s.setData("quantity",Math.max(1,s.data.quantity-1))}function M(){const a=r.stock??1e3;s.setData("quantity",Math.min(a,Number(s.data.quantity)+1))}const n=r.reviews&&r.reviews.length?r.reviews.reduce((a,i)=>a+i.rating,0)/r.reviews.length:0,D=(Number(r.price)*Number(s.data.quantity||1)).toFixed(2);return e.jsxs(e.Fragment,{children:[e.jsx(F,{title:r.name}),e.jsx("style",{children:`
