...
export default function Edit({ product, categories }) {
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        post(route("talent.products.update", product.id), {
            preserveScroll: true,
        });
    };

    const imageUrl = product.image ? `/image/products/${product.image}` : 'https://via.placeholder.com/150';
            ...
            <div className="container-fluid px-4 py-4">
                <div className="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        ...
                        <p className="text-secondary mb-0 small">Update the details for {product.name}</p>
                    </div>
                        ...
                        <form onSubmit={onSubmit}>
                            <div className="row g-4">
                                ...
                                <div className="col-12">
                                    ...
                                    <textarea rows="4" className={`form-control ${errors.description ? 'is-invalid' : ''}`} value={data.description} onChange={(e) => setData('description', e.target.value)}></textarea>
                                    {errors.description && <div className="invalid-feedback">{errors.description}</div>}
                                </div>

                                <div className="col-12">
                                    <label className="form-label fw-semibold d-block">Product Image</label>
                                    <div className="d-flex align-items-center gap-3">
                                        <img src={imageUrl} alt="Product" className="img-thumbnail" style={{maxWidth: '120px', maxHeight: '120px', objectFit: 'cover'}} />
                            ...
                            <div className="mt-4 d-flex justify-content-end">
                                <button type="submit" className="btn btn-dark btn-lg px-4" disabled={processing}>
                                    {processing ? (
                                        ...
                                        <><i className="fas fa-check me-2"></i> Update Product</>
                                    )}
                                </button>
                            </div>
                        </form>
