        ...
        <AdminLayout>
            ...
            <div data-h-scope="profile" className="profile-page">
                ...
                <div className="hero-band">
                    {talent.image ? (
                        <img src={talent.image} alt={talent.name} className="hero-avatar" />
                    ) : (
                        <div className="hero-avatar-placeholder">{talent.name?.charAt(0)?.toUpperCase()}</div>
                    )}

                    <div className="hero-info">
                        <div className="hero-badges">
                            <span className={`badge badge-${status}`}>
                                <span className="badge-dot" />{cap(status)}
                            </span>
                            {talent.featured && <span className="badge-featured">★ Featured</span>}
                            {talent.level && <span className="level-pill">{cap(talent.level)}</span>}
                            {talent.matched && <span className="badge-matched">✓ Matched</span>}
                        </div>
                        ...
                        <div className="hero-meta">
                            ...
                            <div className="hero-meta-item">
                                <CalendarIcon /> Joined {formatDate(talent.created_at)}
                            </div>
                        </div>
                    </div>

                    <div className="hero-actions">
                        <Link href={routes.edit(talent.id)} className="btn-accent">
                            <PencilIcon /> Edit
                        </Link>
                        <Link href={routes.index()} className="btn-secondary">← Back</Link>
                        <button type="button" className="btn-danger-outline" onClick={destroyTalent}>
                            <TrashIcon /> Delete
                        </button>
                    </div>
                </div>
                ...
                <div className="stat-grid">
                    <MiniStat tone="accent" icon={<SkillIcon />} value={talent.skills?.length ?? 0} label="Skills" />
                    <MiniStat tone="info" icon={<BookIcon />} value={talent.stories?.length ?? 0} label="Stories" />
                    <MiniStat tone="gold" icon={<ChatIcon />} value={talent.feedback?.length ?? 0} label="Feedback" />
                    <MiniStat tone="purple" icon={<LinkIcon />} value={talent.connections?.length ?? 0} label="Connections" />
                    <MiniStat tone="danger" icon={<CapIcon />} value={talent.courses?.length ?? 0} label="Courses" />
                    <MiniStat tone="warning" icon={<HeartIcon />} value={talent.supports?.length ?? 0} label="Supports" />
                </div>
                ...
                <div className="content-grid">
                    ...
                    <div className="col-left">
                        <Card icon={<UserIcon />} title="Profile Information">
                            <InfoRow label="Name" value={talent.name} />
                            <InfoRow label="Email" value={talent.email} empty="Not provided" />
                            <InfoRow label="Phone" value={talent.phone} empty="Not provided" />
                            <InfoRow label="Address" value={talent.address} empty="Not provided" />
                            <InfoRow label="Language" value={talent.language} empty="Not specified" />
                            <InfoRow label="Category" value={talent.category?.name} empty="—" />
                            <InfoRow
                                label="Level"
                                value={talent.level ? <span className="level-pill">{cap(talent.level)}</span> : null}
                                empty="Not specified"
                            />
                        </Card>

                        {talent.description && (
                            <Card icon={<TextIcon />} title="Bio / Description" noPadding>
                                <div className="description-text">{talent.description}</div>
                            </Card>
                        )}

                        <Card icon={<SkillIcon />} title="Skills" count={talent.skills?.length ?? 0} noPadding>
                            {talent.skills?.length > 0 ? (
                                ...
                                <SubEmpty icon={<QuestionIcon />} text="No skills added yet" />
                            )}
                        </Card>

                        <Card icon={<BookIcon />} tone="info" title="Stories" count={talent.stories?.length ?? 0} noPadding>
                            {talent.stories?.length > 0 ? (
                                talent.stories.slice(0, 5).map((story) => (
                                    <div className="story-item" key={story.id}>
                                        {story.image && <img src={story.image} alt="" className="story-thumb" />}
                                        <div>
                                ...
                                <SubEmpty icon={<FolderIcon />} text="No stories yet" />
                            )}
                        </Card>
                    </div>
                    ...
                    <div className="col-right">
                        <Card icon={<ChatIcon />} tone="gold" title="Feedback" count={talent.feedback?.length ?? 0} noPadding>
                            {talent.feedback?.length > 0 ? (
                                talent.feedback.slice(0, 4).map((fb) => (
                                    <div className="feedback-item" key={fb.id}>
                                        <div className="feedback-header">
                                            <span className="feedback-author">{fb.name ?? 'Anonymous'}</span>
                                            <span className="feedback-date">{formatDate(fb.created_at)}</span>
                                        </div>
                                        {fb.rating != null && (
                                            <div className="stars">
                                                {Array.from({ length: 5 }, (_, i) => (i < fb.rating ? '★' : '☆')).join('')}
                                            </div>
                                        )}
                                        <div className="feedback-text">{limit(fb.message ?? fb.comment ?? '', 120)}</div>
                                    </div>
                                ))
                            ) : (
                                <SubEmpty icon={<ChatIcon />} text="No feedback yet" />
                            )}
                        </Card>

                        <Card icon={<LinkIcon />} tone="purple" title="Connections" count={talent.connections?.length ?? 0} noPadding>
                            {talent.connections?.length > 0 ? (
                                talent.connections.slice(0, 6).map((conn) => (
                                    <div className="connection-item" key={conn.id}>
                                        <div>
                                            <div className="conn-name">{conn.name ?? `Connection #${conn.id}`}</div>
                                            <div className="conn-type">{conn.type ?? 'General'}</div>
                                        </div>
                                        <span className={`conn-status ${conn.status === 'active' ? 'conn-active' : 'conn-pending'}`}>
                                            {cap(conn.status ?? 'pending')}
                                        </span>
                                    </div>
                                ))
                            ) : (
                                <SubEmpty icon={<LinkIcon />} text="No connections yet" />
                            )}
                        </Card>

                        <Card icon={<ClockIcon />} tone="muted" title="Record Info">
                            <InfoRow label="Record ID" value={<span className="mono">#{talent.id}</span>} />
                            <InfoRow label="Created" value={<span className="sm">{formatDate(talent.created_at, true)}</span>} />
                            <InfoRow label="Updated" value={<span className="sm">{timeAgo(talent.updated_at)}</span>} />
                            {talent.user && <InfoRow label="Owner" value={talent.user.name} />}
                        </Card>
                    </div>
                </div>
            </div>
...
function Card({ icon, tone = 'accent', title, count, noPadding = false, children }) {
    return (
        <div className="ui-card">
            <div className="card-header">
                <div className="card-header-left">
                    <div className="card-header-icon" data-tone={tone}>{icon}</div>
                    <h2>{title}</h2>
                </div>
...
function InfoRow({ label, value, empty = '—' }) {
    ...
    return (
        <div className="info-row">
            ...
            <span className={`info-value ${isEmpty ? 'empty' : ''}`}>{isEmpty ? empty : value}</span>
        </div>
    );
}
...
/* ── Inline icons ── */
function CheckIcon() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>; }
function MailIcon() { return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" /></svg>; }
...
function LinkIcon() { return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" /></svg>; }
function CapIcon() { return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M4.26 10.147a60.436 60.436 0 00-.491 6.347A48.627 48.627 0 0112 20.904a48.627 48.627 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.57 50.57 0 00-2.658-.813A59.905 59.905 0 0112 3.493a59.902 59.902 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.697 50.697 0 0112 13.489a50.702 50.702 0 017.74-3.342M6.75 15a.75.75 0 100-1.5.75.75 0 000 1.5zm0 0v-3.675A55.378 55.378 0 0112 8.443m-7.007 11.55A5.981 5.981 0 006.75 15.75v-1.5" /></svg>; }
...
function UserIcon() { return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" /></svg>; }
function TextIcon() { return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25H12" /></svg>; }
function QuestionIcon() { return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" /></svg>; }
function FolderIcon() { return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" /></svg>; }
function ClockIcon() { return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>; }

/* ── Styles ── shares tokens with the Skills Registry index for visual
   consistency; adds hero-band, stat mini-cards, and content-grid layout. */
const css = `
