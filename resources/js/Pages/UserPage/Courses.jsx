...
/**
 * Converted from resources/views/.../courses-by-category.blade.php
 *
 * Assumptions made during conversion:
 * - `courses` here is a plain collection, not a paginator (the Blade file calls
 *   `$courses->count()` directly with no `hasPages()`/pagination markup), so it's read
 *   as a plain array — no `.data` unwrapping needed, unlike the Learning Center page.
 * - `category.courses.count()` -> `category.courses_count` (assumes
 *   `Category::withCount('courses')->get()` on the backend), falling back to
 *   `category.courses?.length` if you're sending the full loaded relation instead —
 *   same assumption as the main Learning Center conversion.
 * - `course.feedback.avg('rating')` / `.count()` -> `course.avg_rating` /
 *   `course.reviews_count`, falling back to computing from a loaded `course.feedback`
 *   array — same pattern as the main Learning Center page.
 * - `url('/courses/category/'.$category->slug)` had no named route, so it stays a plain
 *   path string via `<Link>` (`/courses/category/${category.slug}`), same as before.
 * - Filter tabs (All / Latest / Popular / Featured) are client-side show/hide against
 *   `course.tag`, now via React state instead of DOM class/style toggling — same
 *   behavior as the original script.
 * - No light theme existed in the original file — added `[data-h-theme="light"]`
 *   overrides using the same --bg/--bg2/--bg3/--border/--green token pattern as your
 *   main Learning Center page, since this listing shares its visual language.
 */
export default function CoursesByCategory({ categoryName, categories = [], courses = [] }) {
  ...
  return (
    <>
      <Head title={`${categoryName} Learning Material`} />
      ...
      <div className="page-header">
        <div className="container">
          <div className="ph-breadcrumb">
            ...
            <Link href={route('user.courses')}>Courses</Link>
            <span className="sep">/</span>
            <span className="current">{categoryName}</span>
          </div>
          <div className="ph-eyebrow">Category</div>
          <h1>
            <span className="accent">{categoryName}</span> Learning Materials
          </h1>
          ...
          <div className="cat-scroll">
            {categories.map((category) => (
              <Link
                ...
                className={`cat-chip${category.name === categoryName ? ' active' : ''}`}
              >
                {category.name}
          ...
          <div className="row g-4">
            ...
            <div className="col-lg-3 d-none d-lg-block">
              ...
              <div className="sidebar-card" style={{ borderColor: 'rgba(0,166,103,0.3)', background: 'var(--green-dim)' }}>
                ...
                <div style={{ fontFamily: 'var(--font-head)', fontSize: '1.05rem', fontWeight: 800, color: 'var(--white)' }}>{categoryName}</div>
                <div style={{ fontSize: '0.78rem', color: 'var(--muted)', marginTop: '4px' }}>{courses.length} course(s) available</div>
              </div>

              <div className="sidebar-card">
                <div className="sidebar-title"><i className="ti ti-layout-grid"></i> All Categories</div>
                {categories.map((category) => (
                  <Link
                    key={category.id}
                    href={`/courses/category/${category.slug}`}
                    className={`sidebar-cat-link${category.name === categoryName ? ' active' : ''}`}
                  >
                    {category.name}
              ...
              <Link href={route('user.courses')} className="btn-green" style={{ width: '100%', justifyContent: 'center', marginTop: '4px' }}>
                <i className="ti ti-arrow-left"></i> All Courses
              </Link>
            ...
            <div className="col-lg-9">
              ...
              <div className="d-flex align-items-end justify-content-between flex-wrap gap-3 mb-4">
                <div>
                  <div className="section-label">{categoryName}</div>
                  <div className="section-title">Browse Courses</div>
                  <p className="section-sub">Learning center &amp; courses listing</p>
                </div>
                <div className="d-lg-none">
                  <Link href={route('user.courses')} className="btn-green" style={{ padding: '9px 16px', fontSize: '0.8rem' }}>
                    <i className="ti ti-arrow-left"></i> All Courses
                  </Link>
                </div>
              </div>
              ...
              <div className="filter-bar">
                ...
                <span className="result-count d-none d-md-block">
                  <strong>{courses.length}</strong> course(s) in {categoryName}
                </span>
              </div>
                ...
                {visibleCourses.length > 0 ? (
                  ...
                  <div className="empty-state">
                    ...
                    <Link href={route('user.courses')} className="btn-green mt-3" style={{ margin: '0 auto' }}>
                      <i className="ti ti-arrow-left"></i> Browse All Courses
                    </Link>
                  </div>
                )}
              </div>
...
CoursesByCategory.layout = (page) => (
  <GuestLayout
    children={page}
    title={`${page.props.categoryName} Learning Material`}
    description="View all learning material and courses offered by skilled people in this category."
  />
);
