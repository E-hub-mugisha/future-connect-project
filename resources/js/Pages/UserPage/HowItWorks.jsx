...
/**
 * Converted from resources/views/.../how-it-works.blade.php
 *
 * Assumptions made during conversion:
 *
 * 1. This Blade file only had a "@section('content')" with no hero/categories/
 *    feature-tabs/partners/CTA markup actually present in the body — despite full
 *    CSS being defined for all of those sections (.fc-hero, .fc-categories,
 *    .fc-features, .fc-partners, .fc-testimonials, .fc-cta, etc). Only 4 sections
 *    had actual HTML: the "How It Works" 3-step strip, the feature strip, the
 *    tabbed "How it Works" (Skills vs Client/Employer) cards, testimonials, and the
 *    CTA banner. I carried over every CSS rule as-is (including the unused blocks)
 *    since removing "dead" CSS wasn't requested and it's harmless, but only
 *    rendered the sections that had matching markup in the Blade file.
 *
 * 2. Bootstrap tabs (`data-bs-toggle="tab"`) — kept as native Bootstrap markup
 *    rather than rewriting as controlled React state, since your project already
 *    loads Bootstrap's JS bundle (needed elsewhere for the accordion `.fc-accordion`
 *    classes in the CSS). If Bootstrap's JS isn't globally initialized in this
 *    Inertia app, let me know and I'll convert this to a `useState`-driven tab
 *    instead.
 *
 * 3. Owl Carousel (`.owl-carousel`, `.owl-stage`, `.owl-nav`) — the original relied
 *    on the Owl Carousel jQuery plugin to compute stage width/transform inline
 *    (`transform: translate3d(-1320px,...)`, `width: 4400px`) based on the exact
 *    item count, and to clone slides for infinite loop (`owl-item cloned`). None of
 *    that is reproducible statically/correctly in React without also porting in the
 *    jQuery plugin and calling `.owlCarousel()` in a `useEffect`, which felt out of
 *    scope for a straight Blade→JSX conversion. Since the actual list content is
 *    just review cards, I rendered them as a horizontally-scrollable flex row
 *    (same `category-scroll`-style pattern I've used on your other converted pages)
 *    using the existing `.review-card` styling, and dropped the owl-specific
 *    wrapper/nav/dots. Let me know if you'd rather I wire up a real carousel
 *    library (e.g. `embla-carousel-react`, already common in React/Inertia stacks)
 *    for infinite-loop + arrow-nav behavior matching the original.
 *
 * 4. `Str::limit($story->excerpt, 80)` — reimplemented as a small local
 *    `truncate()` helper (char-based, matches Laravel's default `Str::limit`
 *    behavior of appending `...`).
 *
 * 5. `route('user.register_skills')`, `route('user.talents')`,
 *    `route('talent.connections-room')`, `route('user.jobs.index')` all carried
 *    over as-is via Ziggy's global `route()`.
 *
 * 6. `successStories` is expected as a plain array/collection prop (not a
 *    paginator) — matches the `@forelse` used in the original, so I used a plain
 *    `.length === 0` check for the empty state rather than the paginator-aware
 *    pattern from your other pages.
 */
...
export default function HowItWorks({ successStories = [] }) {
  return (
    <>
      <Head title="How It Works | Future Connect" />
        ...
        <div className="container">
          ...
          <div className="steps-grid">
            <div className="step-card">
              ...
              <Link href={route('user.register_skills')} className="strip-link">
            ...
            <div className="step-card">
              ...
              <Link href={route('user.talents')} className="strip-link">
                Explore Skills <i className="ti ti-arrow-right"></i>
              </Link>
            </div>
            <div className="step-card">
              ...
              <Link href={route('talent.connections-room')} className="strip-link">
        ...
        <div className="container">
          <div className="feature-strip-grid">
            <div className="feature-strip-item">
              <div className="strip-icon"><i className="ti ti-rocket"></i></div>
              <h5>Skills Marketplace</h5>
              <p>Stand out and reach 3× more employers. Boost your profile, get verified, and feature your story.</p>
              <Link href={route('user.talents')} className="strip-link">
                Find Skills <i className="ti ti-arrow-right"></i>
              </Link>
            </div>
            <div className="feature-strip-item">
              <div className="strip-icon"><i className="ti ti-briefcase"></i></div>
              ...
              <Link href={route('user.jobs.index')} className="strip-link">
                Start Exploring <i className="ti ti-arrow-right"></i>
              </Link>
            </div>
            <div className="feature-strip-item">
              <div className="strip-icon"><i className="ti ti-users"></i></div>
              ...
              <Link href={route('talent.connections-room')} className="strip-link">
                Skill Connect<i className="ti ti-arrow-right"></i>
              </Link>
            </div>
          </div>
        </div>
      ...
      <section className="why-choose-sec">
        <div className="container">
          ...
          <ul className="nav nav-tabs justify-content-center mb-4" id="howItWorksTab" role="tablist">
            <li className="nav-item" role="presentation">
              <button
                className="nav-link active"
                id="talent-tab"
                data-bs-toggle="tab"
                data-bs-target="#talent"
                type="button"
                role="tab"
          ...
          <div className="tab-content" id="howItWorksTabContent">
            {/* Talent Tab */}
            <div className="tab-pane fade show active" id="talent" role="tabpanel" aria-labelledby="talent-tab">
              <div className="row">
          ...
          <div className="row">
            <div className="col-md-12">
              {successStories.length > 0 ? (
                ...
                <p className="text-muted text-center w-100">No success stories yet. Check back soon!</p>
              )}
            </div>
          </div>
        </div>
            ...
            <div className="row gx-0 align-items-center">
              <div className="col-md-8">
                <div className="cta-info">
                  <span className="badge">Ready to Get Started?</span>
                  <h3>Join Future Connect today and unlock your potential.</h3>
                </div>
              </div>
              <div className="col-md-4 text-md-end">
                <div className="cta-btn">
                  <Link href={route('user.register_skills')} className="btn btn-primary btn-lg">
                    Register your Skills <i className="ti ti-arrow-badge-right"></i>
                  </Link>
                </div>
              </div>
            </div>
            <div className="cta-bg">
              <div className="cta-bg1"><img src="/assets/img/bg/contact-bg-01.png" alt="" /></div>
              <div className="cta-bg2"><img src="/assets/img/bg/contact-bg-02.png" alt="" /></div>
            </div>
          </div>
        </div>
      </div>
...
HowItWorks.layout = (page) => (
  <GuestLayout children={page} title="How It Works | Future Connect" />
);
