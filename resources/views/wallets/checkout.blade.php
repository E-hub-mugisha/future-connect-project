...
<div class="container">
    <div class="payment-card">
        <h3 class="payment-title mb-3">Wallet Top-Up</h3>
        <p class="payment-desc">
            You are going to top up <strong>{{ number_format($transaction->amount, 2) }} RWF</strong> to your wallet.
        </p>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        payBtn.addEventListener("click", function() {
            ...
            const txRef = transactionId + "-" + Date.now();
            ...
            FlutterwaveCheckout({
                ...
                callback: function(data) {
                    window.location.href = `/wallets/topup/callback?transaction_id=${transactionId}&email=${encodeURIComponent(userEmail)}&status=${encodeURIComponent(data.status)}&tx_ref=${encodeURIComponent(data.tx_ref)}`;
                },
                onclose: function() {
                    payBtnText.innerHTML = '<i class="fa fa-money-bill-wave"></i> Pay Now';
                    payBtnSpinner.classList.add("d-none");
                ...
                customizations: {
                    title: "Wallet Top-Up",
                    description: "Pay to top up your wallet",
                    logo: "{{ asset('logo.png') }}"
                }
            });
        });
